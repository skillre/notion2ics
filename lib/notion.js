...
const fieldMappings = {
  ...
  // 人员字段 - 默认为"一线人员"
  personnelField: process.env.NOTION_PERSONNEL_FIELD || '一线人员'
};
...
/**
 * 获取人员列表
 * @returns {Promise<Array>} 人员列表 [{id, name}]
 */
export async function getPersonnelList() {
  try {
    ...
    // 1. 获取数据库Schema以找到关联的人员数据库ID
    const dbResponse = await notion.databases.retrieve({ database_id: databaseId });
    const personnelProp = dbResponse.properties[fieldMappings.personnelField];

    if (!personnelProp || personnelProp.type !== 'relation') {
      console.warn(`未找到名为"${fieldMappings.personnelField}"的关系字段，无法获取人员列表`);
      return [];
    }

    const personnelDbId = personnelProp.relation.database_id;
    console.log(`找到人员数据库ID: ${personnelDbId}`);

    // 2. 查询人员数据库获取所有人员
    // 注意：这里假设人员数据库有一个Title字段作为姓名
    const response = await notion.databases.query({
      database_id: personnelDbId,
      sorts: [
        {
    ...
    // 3. 格式化返回结果
    const personnel = response.results.map(page => {
      // 寻找title类型的属性作为名称
      const titleProp = Object.values(page.properties).find(prop => prop.type === 'title');
      const name = titleProp?.title?.[0]?.plain_text || '未命名';

      return {
        id: page.id,
        name: name
      };
    }).filter(p => p.name !== '未命名'); // 过滤掉无效条目

    console.log(`获取到 ${personnel.length} 名人员`);
    return personnel;

  } catch (error) {
    console.error('获取人员列表失败:', error);
    // 不抛出错误，而是返回空列表，以免影响主功能
    return [];
  }
}
...
/**
 * 从Notion数据库获取所有日历事件
 * @param {string} personId - 可选，人员ID用于过滤
 * @returns {Promise<Array>} 事件列表
 */
export async function getCalendarEvents(personId = null) {
  try {
    const databaseId = process.env.NOTION_DATABASE_ID;
    ...
    // 如果提供了personId，添加人员过滤条件
    if (personId) {
      console.log(`添加人员过滤条件，ID: ${personId}`);
      dateFilter.and.push({
        property: fieldMappings.personnelField,
        relation: {
          contains: personId
        }
      });
